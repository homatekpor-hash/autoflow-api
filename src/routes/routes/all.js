...
wsRouter.put("/:id", authenticate, requireRole("OWNER", "MANAGER"), wsCtrl.update);
...
jobRouter.put("/:id/status",        authenticate, requireRole("OWNER","MANAGER","TECHNICIAN"), body("status").notEmpty(), jobCtrl.updateStatus);
jobRouter.put("/:id/assign",        authenticate, requireRole("OWNER","MANAGER"), jobCtrl.assignTechnician);
...
jobRouter.post("/:jobId/estimate",  authenticate, requireRole("OWNER","MANAGER","TECHNICIAN"), require("../controllers/estimateController").create);
...
estRouter.put("/:id",              authenticate, requireRole("OWNER","MANAGER","TECHNICIAN"), estCtrl.update);
estRouter.post("/:id/send",        authenticate, requireRole("OWNER","MANAGER","TECHNICIAN"), estCtrl.send);
...
userRouter.post("/invite",  authenticate, requireRole("OWNER","MANAGER"),
  body("name").notEmpty(), body("email").isEmail(), body("role").notEmpty(),
  userCtrl.invite
);
userRouter.put("/:id",      authenticate, requireRole("OWNER","MANAGER"), userCtrl.update);
...
repRouter.get("/revenue",     authenticate, requireRole("OWNER","MANAGER"), repCtrl.revenue);
repRouter.get("/jobs",        authenticate, requireRole("OWNER","MANAGER"), repCtrl.jobs);
repRouter.get("/performance", authenticate, requireRole("OWNER","MANAGER"), repCtrl.performance);
